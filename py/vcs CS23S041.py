...
class NetworkAdmin:
    ...
    def assign_vcid(self, path,conn_id):
        vcid_list = []
        for i in range(len(path) - 1):
            incoming_link = (path[i], path[i+1])
            outgoing_link = (path[i+1], path[i])

            vcid = self.vc_table.get(conn_id)
            if vcid is None:
    ...
    def optimistic_admission(self, connection, path, edges):
        ...
        for i in range(len(path) - 1):
            ...
            if b_iequiv > edges[(link[0], link[1])].capacity - self.link_utilization[link]:
                return False
        vcid_list = self.assign_vcid(path,connection.id)
        for i in range(len(path) - 1):
            link = (path[i], path[i+1])
...
def parse_topology(file, weight_type):
    with open(file, 'r') as f:  
        ...
        for _ in range(edge_count):
            ...
            if not line:
                continue
            src, dest, delay, capacity = map(int, line.split())
            ...
            edges[(src, dest)] = edge
            edges[(dest, src)] = edge
            #print(edge.weight)  # Assuming bidirectional
...
def parse_connections(file):
    with open(file, 'r') as f:
        ...
        for _ in range(connection_count):
            ...
            if not line:
            	continue
            src, dest, bimin, biave, bimax = map(int, line.split())
    ...
    while pq:
        ...
        for edge in vertices[node].edges:
            ...
            if neighbor not in visited:
                heapq.heappush(pq, (cost + edge.weight, neighbor, path))
    return (float("inf"), [])  # Return an empty path instead of None
...
def find_two_shortest_paths(vertices, src, dest):
    ...
    # Create a temporary graph with edges from the first path removed
    temp_vertices = {k: Vertex(k) for k in vertices}
    for v in temp_vertices:
        temp_vertices[v].edges = [
            edge for edge in vertices[v].edges 
            if not ((edge.src in path1 and edge.dest in path1) or (edge.dest in path1 and edge.src in path1))
        ]
...
def can_admit_connection(admin, connection, paths, edges, p_value):
    for path_cost, path in paths:
        if not path:  # Check if the path is valid
            continue
...
def process_virtual_circuit_switching(topology_file, connection_file, p_value, flag_value):
    ...
    for conn_id, request in enumerate(requests):
        ...
        if admitted:
            ...
            for i in range(len(path) - 1):
                if i == 0:
                    forwarding_table[path[i]].append((path[i],path[i], path[i+1], vcid_list[i], path[i+1], vcid_list[i]))
                forwarding_table[path[i]].append((path[i-1],path[i], path[i+1], vcid_list[i], path[i+1], vcid_list[i]))
            #print(forwarding_table.items())
        ...
        for node, routes in routing_table.items():
            ...
            for dest, path, delay, cost in routes:
                if flag_value == 'dist':
                    ...
                    rt.write(f"Dest: {dest}, Path: {' -> '.join(map(str, path))}, Path Cost: {cost}\n")
            rt.write("\n")
    ...
    with open(args.forwarding, 'w') as ft:
        for node, forwards in forwarding_table.items():
            ...
            for in_node,src, dest, in_vcid, out_node, out_vcid in forwards:
                ft.write(f"This Router's ID: {src}, Incoming Port: {in_node}, VC ID: {in_vcid}, Outgoing Port: {out_node}, VC ID: {out_vcid}\n")
            ft.write("\n")
...
def main():
    ...
    process_virtual_circuit_switching(args.topology, args.connections, p_value=args.pflag, flag_value=args.flag)
